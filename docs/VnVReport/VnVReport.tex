...
\hline
31/03/2024 & 2.0 & Fixed table appearances, GitHub stylizing, added table rows describing test groupings, explicitly mentioned tables, alphabetized symbols table, backend performance graph fix, link stakeholder to SRS, FRT-FR5-2 details, sub-test details fix,\\
\hline
\end{tabularx}
...
the Functional Requirements, Nonfunctional Requirements and stakeholders can be found in the
...
\begin{longtable}{|l|c|p{0.65\linewidth}|}
  ...
  \hline
  \multicolumn{3}{|c|}{\small \textit{Login process tests}} \\
  \hline
  ...
  \hline
  \multicolumn{3}{|c|}{\small \textit{User credential case coverage tests}} \\
  \hline
  FRT-FR2-1 & $\checkmark$ & \\
  \hline
  FRT-FR2-2 & $\checkmark$ & \\
...
Table 1 shows test results for user authentication testing. FRT-FR1-1 to FRT-FR2-7 were all manually performed tests to verify the login and authentication process.
...
\begin{longtable}{|l|c|p{0.65\linewidth}|}
  ...
  \textbf{Test ID} & \textbf{Result} & \textbf{Details} \\
  \hline
  \multicolumn{3}{|c|}{\small \textit{OCR accuracy tests}} \\
  \hline
  FRT-FR3-1 & $\checkmark$ & 95\% accuracy from manual testing\\
  \hline
  \multicolumn{3}{|c|}{\small \textit{Receipt scanning process edge case tests}} \\
  \hline
...
Table 2 shows test results for manually performed tests primarily pertaining to the receipt scanning process.
...
\begin{longtable}{|l|c|p{0.65\linewidth}|}
  ...
  \textbf{Test ID} & \textbf{Result} & \textbf{Details} \\
  \hline
  \multicolumn{3}{|c|}{\small \textit{Add goal tests}} \\
  \hline
  FRT-FR5-1 & $\checkmark$ & \\
  \hline
  \multicolumn{3}{|c|}{\small \textit{Edit goal tests}} \\
  \hline
  FRT-FR5-2 & X & Unimplemented functionality, to be completed for Revision 1\\
  \hline
\end{longtable}

Table 3 shows test results for manual tests verifying the User Goals functionality of the app. FRT-FR5-1 tests the ability
...
\begin{longtable}{|l|c|p{0.65\linewidth}|}
  ...
  \textbf{Test ID} & \textbf{Result} & \textbf{Details} \\
  \hline
  \multicolumn{3}{|c|}{\small \textit{Input to product matching tests}} \\
  \hline
  FRT-FR7-1 & $\checkmark$ & \\
  \hline
  FRT-FR7-2 & $\checkmark$ & \\
  ...
  FRT-FR7-3 & $\checkmark$ & \\
  \hline
  \multicolumn{3}{|c|}{\small \textit{Recommendation engine tests}} \\
  \hline
  FRT-FR8-1 & $\checkmark$ & \\
  \hline
  ...
  FRT-FR9-2 & $\checkmark$ & \\
  \hline
  \multicolumn{3}{|c|}{\small \textit{User submission validation tests}} \\
  \hline
...
Table 4 shows test results for verifying how the system responds to data input, such
...
Table 5 shows test results for manually performed by external users under the supervision of the team.
...
Table 6 shows test results for usability performed with external users with supervision and instruction from members of the team. NFRT-UH1 was a test
...
Table 7 shows test results for performance done by the team using various tools like Flutter Performance Profiling and Postman
...
backend performance was within expectation for the project (different colours are simply to distinguish between trials). Most of the responses were within 200 ms with the
...
Table 8 shows test results for cultural tests performed internally by team members. NFRT-CUR2 verifies the Classification Engine
...
Table 9 shows test results for operations and environment tests performed internally by team members. NFRT-OER1 verifies the compatibility of the application
...
Table 10 shows test results for security tests performed internally by team members. NFRT-SR2 is a test that verifies protections are properly put in
...
Table 11 shows results for the receipt extraction module. To summarize, tests FRT-M3-1 to FRT-M3-26 are all frontend tests. These tests focus on the helper functions used
...
These tests primarily focus on the endpoints responsible for CRUD operations related to user budget goals. Each of these tests require all sub-tests (a, b, c, etc.) to pass that test the function's primary and edge-cases.
...
Table 13 shows test results for the users module. Tests FRT-M6-1 to FRT-M6-6 are all back-end tests and can be found in the following file: \href{https://github.com/grocery-spending-tracker/grocery-spending-tracker-backend/blob/master/tests/controllers/usersController.users.test.js}{Back-end User Module Controller Tests}.
These tests focus on the endpoints related to CRUD operations for users on the application. Each test requires all sub-tests (a, b, c, etc.) to pass that test the function's primary and edge-cases.
...
Table 14 shows test results for the authentication module. Tests FRT-M7-1 to FRT-M7-5 are the front-end unit tests for the Authentication Module. These tests focus on the user flows for authenticating user identity on the mobile app. Details on these tests can be found in the following file: \href{https://github.com/allanfang1/grocery_spending_tracker_app/blob/main/test/profile_provider_test.dart}{Front-end Authentication Module Tests}.
...
Each test has requires all sub-tests (a, b, c, etc.) to pass that test the function's primary and edge-cases.
...
Table 15 shows test results for the recommendation engine. Tests FRT-M8-1 and FRT-M8-2 are both back-end tests and can be found in the following file: \href{https://github.com/grocery-spending-tracker/grocery-spending-tracker-backend/blob/314b/unit-test/tests/controllers/recommendationController.test.js}{Back-end Recommendation Module Controller Tests}.
FRT-M8-1 focuses on the endpoint for returning recommendations based on a user's frequency of purchase and FRT-M8-2
focuses on the endpoint for returning recommendations based on low product prices. Each test require all sub-tests (a, b, c, etc) to pass that test the function's primary and edge-cases.
...
Table 16 shows test results for the classification engine. Tests FRT-M9-1 to FRT-M9-11 are all back-end tests and can be found in the following file:
...
Each test requires all sub-tests (a, b, c, etc) to pass that tests the function's primary and edge-cases.
