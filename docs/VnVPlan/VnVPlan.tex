...
How test will be performed: The team will install the application on various mobile devices running Android on version 13 (IASR2) and iOS on version 16 (IASR1). The team will also test on various tablet devices running Android or iPadOS. The team will confirm that all functionalities work as expected on each device by conducting variety of actions and system events.

\item{NFRT-OER2\\}

Type: Functional, Dynamic, Manual

Initial State: The application is installed on device that have operable camera peripherals and access to the internet.
...
How test will be performed: The team will test the application on devices with and without camera peripherals to validate (WER1). The team will disconnect devices from the internet to test offline functionality. The team will then reconnect the device to the internet to test online features (WER2). The team will confirm that all functionalities work on devices that have access to camera peripherals and internet access.
...
Input: The user will attempt to execute test suites against new releases and updates.
...
How test will be performed: The team will use automated testing tools (that can include \textit{Appium}, \textit{LoadRunner}, \textit{Jenkins}, \textit{Github CI/CD}, etc) to achieve 80\% code coverage (RER1). The team will then perform issue tracking and resolution processes (RER2). The team will organize a Final Demonstration (RER3) between March 18 and March 29 of 2024, involving stakeholders to validate the final product against requirements.
