...
\item output: \( \textit{out} \coloneqq location \), where textit{location} is a String representing the user location stored in the database (Section \ref{mDBDriver})
\item exception: \( \textit{exc} \coloneqq noLocationSavedException \), if the database (Section \ref{mDBDriver}) has no value saved for user location
\end{itemize}
