...
\begin{itemize}
\item transition: none
\item output: \( userExists(\textit{user.userId}) \Rightarrow getUserInDB(userId)\)
...
\begin{itemize}
\item transition: none
\item output: \( userExists(\textit{user.userId}) \Rightarrow \) user, from database (Section \ref{mDBDriver}).
\item exception \( \textit{exc} \coloneqq \neg userExists(\textit{user.userId}) \Rightarrow \\ userDneException\)
\end{itemize}
...
\begin{itemize}
\item transition: none
\item output: \( getTripFromDateTime(\textit{dateTime}) \)
\end{itemize}
