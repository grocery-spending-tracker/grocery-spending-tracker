...
\textbf{Use Case:} 6, 7, 8, 9\\
\textbf{Description:} The system must allow users to verify and manually edit or enter grocery items that were not accurately scanned and recognized by the system.\\
\textbf{Rationale:} It is essential for the system to ensure data accuracy and completeness of tracking of user grocery purchases. Although scanning and recognizing image text using OCR have become advanced, it is impossible to ensure 100% accuracy of results all the time. Therefore, allowing users to manually verify and update items will enhance reliability of data.\\
\textbf{Fit Criterion:} The system should reflect manual changes made by users on the purchase items log in less than 3 seconds. Also, user feedback on ease of manual entry functions should be positive, with more than 80% of users rating it above easy.\\
